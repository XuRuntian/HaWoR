"""Fresh-process execution with complete command, stdout/stderr and failure logs."""
...
def main():
    ...
    parent = root/"outputs/pre_frontend_comparison/vio_replacement_full_60s"
    ...
    if out == parent or not out.is_relative_to(parent) or out.is_relative_to(parent/"process_logs"):
        raise ValueError("Use an isolated run directory under the VIO output parent")
    ...
    for stage in stages:
        command = [sys.executable, str(root/"scripts/run_vio_replacement.py"), "--output", str(out), "--stage", stage]
